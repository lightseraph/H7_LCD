...
void _lv_disp_refr_timer(lv_timer_t *tmr)
{
    ...
    if (disp_refr->wallpaper_layer)
        lv_obj_update_layout(disp_refr->wallpaper_layer);
...
static void refr_area_part(lv_draw_ctx_t *draw_ctx)
{
    ...
    lv_obj_t *top_wallpaper_layer = NULL;
    ...
    if (disp_refr->wallpaper_layer)
    {
        top_wallpaper_layer = lv_refr_get_top_obj(draw_ctx->buf_area, disp_refr->wallpaper_layer);
    }
    /*Draw a display background if there is no top object*/
    if (top_act_scr == NULL && top_prev_scr == NULL && top_wallpaper_layer == NULL)
    {
        lv_area_t a;
        lv_area_set(&a, 0, 0,
    ...
    if (disp_refr->draw_prev_over_act)
    {
        ...
        if (disp_refr->wallpaper_layer)
        {
            if (top_wallpaper_layer == NULL)
                top_wallpaper_layer = disp_refr->wallpaper_layer;
            refr_obj_and_children(draw_ctx, top_wallpaper_layer);
        }
